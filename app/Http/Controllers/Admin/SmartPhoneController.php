...
class SmartPhoneController extends Controller
{
    ...
    {
        $data = Product::select('*')->where('type', 5)->orderBy('id', 'desc');
        return DataTables::of($data)
            ...
            ->addColumn('action', function ($data) {
                return '
                        <a href="' . route('admin.smartphone.edit', $data->id) . '" class="btn btn-xs btn-warning"><i class="fa fa-eye"></i> Edit</a>
                        <a data-url_delete="' . route('admin.smartphone.delete', $data->id) . '" class="btn btn-xs btn-danger delete-confirm" ><i class="fa fa-times"></i> Delete</a>';
            })
    ...
    public function create(CategoryRepositoryEloquent $categoryRepository)
    {
        ...
        return view('admin.smartphone.add', $data);
    }
    ...
    {
        $data = array_merge(['type' => 5], $request->only(['code', 'name', 'status', 'year', 'description', 'content', 'price', 'price_old']));
        ...
        Alert::success('Thêm mới điện thoại thành công');
        return redirect()->route('admin.smartphone.index');
    }
    ...
    {
        $data['smartphone'] = $productRepository->find($id);
        $data['categories'] = $categoryRepository->findWhere(['status' => 1, 'type' => 1]);
        $data['categories_id'] = $data['smartphone']->categories->pluck('id')->toArray();

        return view('admin.smartphone.update', $data);
    }
